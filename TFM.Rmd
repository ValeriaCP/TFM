...
```{r}
set.seed(123) 
data_split <- initial_split(mh_dataset1, prop = 0.8)
...
```{r}
set.seed(123)
cv_splits <- vfold_cv(train_data, v = 8)
...
test_knn_metrics <- test_knn_predictions %>%
  ...
  bind_rows(
    test_knn_predictions %>%
      yardstick::spec(Outcome, .pred_class)
  )
...
tuned_log_reg <- tune_grid(
  ...
  resamples = cv_splits,
  grid = log_reg_grid,
 metrics = metric_set(yardstick::accuracy, yardstick::roc_auc, yardstick::sens, yardstick::spec))

log_reg_metrics <- tuned_log_reg %>%
  collect_metrics()
...
```{r}
test_log_metrics <- test_log_predictions %>%
  metrics(truth = Outcome, estimate = .pred_class) %>%
  bind_rows(
...
```{r}
test_boost_metrics <- test_boost_predictions %>%
  metrics(truth = Outcome, estimate = .pred_class) %>%
  bind_rows(
...
```{r}
rf_spec <- rand_forest(
  trees = tune(),  # Start with a moderate number of trees
  mtry = tune(),
  min_n = tune()# Add maximum depth tuning
) %>%
  set_engine("ranger") %>%
  set_mode("classification")
...
rf_grid <- grid_regular(
  ...
  trees(range = c(100, 1000)),  # Moderate number of trees
  min_n(range = c(2, 10)),
  levels = 5
)
```
...
svm_spec <- svm_rbf(
  ...
  rbf_sigma = tune()     # Gamma parameter
) %>%
  set_engine("kernlab") %>%
  set_mode("classification")
...
# Evaluate on the test set
test_metrics_svm <- test_svm_predictions %>%
  metrics(truth = Outcome, estimate = .pred_class) %>%
  bind_rows(
...
test_metrics_nb <- test_nb_predictions %>%
  metrics(truth = Outcome, estimate = .pred_class) %>%
  bind_rows(
    test_nb_predictions %>%
      roc_auc(truth = Outcome, .pred_1)
